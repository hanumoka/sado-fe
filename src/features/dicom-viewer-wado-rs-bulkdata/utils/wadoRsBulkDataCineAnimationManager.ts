/**
 * WadoRsBulkDataCineAnimationManager - WADO-RS BulkData 전용 중앙 집중식 애니메이션 루프 관리자
 *
 * dicom-viewer, dicom-viewer-wado-uri의 cineAnimationManager.ts와 완전 독립적인 구현
 * Phase 2 최적화: React 상태 우회 (Direct Viewport Manipulation)
 */

import type { Types } from '@cornerstonejs/core'
// WADO-RS BulkData 전용 store import (순환 참조 방지를 위해 동적 import 사용)
...
class WadoRsBulkDataCineAnimationManager {
  ...
  /**
   * Viewport 등록 (WadoRsBulkDataSlot에서 viewport 생성 후 호출)
   * @param slotId 슬롯 ID
  ...
  registerViewport(slotId: number, viewport: Types.IStackViewport, totalFrames: number): void {
    ...
    if (DEBUG_CINE) console.log(`[WadoRsBulkDataCineManager] Viewport registered for slot ${slotId}, totalFrames: ${totalFrames}`)
  }

  /**
   * Viewport 등록 해제 (WadoRsBulkDataSlot cleanup 시 호출)
   * @param slotId 슬롯 ID
   */
  unregisterViewport(slotId: number): void {
    this.viewports.delete(slotId)
    if (DEBUG_CINE) console.log(`[WadoRsBulkDataCineManager] Viewport unregistered for slot ${slotId}`)
  }
  ...
  private animate = (currentTime: number): void => {
    ...
    if (elapsed >= this.frameTime) {
      ...
      this.activeSlots.forEach((slotId) => {
        const info = this.viewports.get(slotId)
        if (!info) {
          if (DEBUG_CINE) console.warn(`[WadoRsBulkDataCineManager] No viewport for slot ${slotId}`)
          return
        }
  ...
  start(): void {
    ...
    this.animationId = requestAnimationFrame(this.animate)
    if (DEBUG_CINE) console.log('[WadoRsBulkDataCineManager] Animation loop started')
  }
  ...
  stop(): void {
    if (this.animationId !== null) {
      ...
      if (DEBUG_CINE) console.log('[WadoRsBulkDataCineManager] Animation loop stopped')
    }
  }
  ...
  registerSlot(slotId: number): void {
    ...
    if (DEBUG_CINE) console.log(`[WadoRsBulkDataCineManager] Slot ${slotId} registered, active: ${this.activeSlots.size}`)
  }
  ...
  unregisterSlot(slotId: number): void {
    ...
    const info = this.viewports.get(slotId)
    if (info) {
      useWadoRsBulkDataMultiViewerStore.getState().setSlotFrame(slotId, info.currentIndex)
      if (DEBUG_CINE) console.log(`[WadoRsBulkDataCineManager] Synced frame index ${info.currentIndex} to Zustand for slot ${slotId}`)
    }

    if (this.activeSlots.size === 0) this.stop()
    if (DEBUG_CINE) console.log(`[WadoRsBulkDataCineManager] Slot ${slotId} unregistered, active: ${this.activeSlots.size}`)
  }
