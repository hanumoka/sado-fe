/**
 * wadoRsBulkDataImageLoader.ts
 *
 * WADO-RS BulkData 이미지 로딩을 위한 중복 요청 방지 레이어
 * WADO-URI의 pendingLoads 패턴을 WADO-RS BulkData에 적용
 *
 * 주요 기능:
...
/**
 * WADO-RS BulkData 이미지 로드 (중복 요청 방지)
 *
 * 동일한 imageId에 대한 동시 요청이 있으면
 * 새 네트워크 요청을 생성하지 않고 기존 Promise를 반환
 *
 * @param imageId WADO-RS BulkData imageId (wadors:...)
 * @returns 로드된 이미지
 */
export async function loadWadoRsBulkDataImage(imageId: string): Promise<Types.IImage> {
  totalRequests++
 ...
 * 로더 통계 조회
 */
export function getWadoRsBulkDataLoaderStats(): {
 ...
 * 통계 리셋
 */
export function resetWadoRsBulkDataLoaderStats(): void {
  totalRequests = 0
